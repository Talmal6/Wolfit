...
import { recordPlayerFinishAPI } from '../api/gameApi';
...
const Stopwatch: React.FC<StopwatchProps> = ({ names }) => {
  ...
  // Handle when a player finishes by calling the API
  const handlePlayerFinish = async (playerName: string, time : number, courseID : number) => {
    const result = await recordPlayerFinishAPI(playerName);
    if (result) {
  ...
  return (
    <div className="stopwatch">
      ...
      <div className="controls">
        <button onClick={start} className="btn start">{isRunning ? 'Running' : 'Start'}</button>
              ...
              <div key={index} className="participant" style={{ marginRight: '10px' }}>
                <div className="name-box">
                  ...
                  {playerTimes.get(name) !== undefined ? (
                    ...
                    <button onClick={() => handlePlayerFinish(name, playerTimes.get(name) ?? 0, 0)}>Finish</button>
                  )}
                </div>
              </div>
            ))}
